...
abstract class Biosensor
{
	...
	protected void calibration()
	{
		...
		if(dont_normalize == true)
		{
			minCal = map ( minCal, global_min, global_max, 1, 10 );
			maxCal = map ( maxCal, global_min, global_max, 1, 10 );
		}
	...
	private float normalizeValue(float toNormalize) 
	{
		...
		if(dont_normalize == true)
			normalized = map ( toNormalize, global_min, global_max, 1, 10 );
		else
			normalized = map ( toNormalize, getMin(), getMax(), 1, 10 );
		return normalized;
	}
