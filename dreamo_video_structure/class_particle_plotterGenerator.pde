class PlotterGenerator extends Particle  //<>// //<>// //<>//
{
 ...
 public void update() 
 {
  ...
  if (frameCount % 15 == 0) 
  {
   ...
   {
    setPosition(new Vector2d(random(-width / 6, width / 6), random(-height / 6, height / 6), false));
    damping = 0.001;
   } 
   else if (getParameter(0) > 0.9) 
   {
    connectionRadius = 50;
    zoom = 1.05 * getParameter(0);
    lineWeight = 7;
   }
   if (audio_decisor.getInstantFeatures()[1] > 0.46) 
   {
    lineWeight = 40;
    connectionRadius = 10;
    particleConnectionRadius = 500; /*setRotation(random(PI)/8);*/
   }
 ...
 void loadPointsFromTxt() 
 {
  ...
  if (path.equals(null)) 
  {
   println("ERROR: loadPointsFromTxt - txt file path is not valid");
   return;
  }
 ...
 public void loadPointsFromFile(File inputFile) 
 {
  if (inputFile == null) 
  {
   println("ERROR: loadPointsFromFile called with NULL argument - could not open the txt file");
   return;
  }
 ...
 return (
  (
   ...
   !(global_stage.getCurrentScene().getParticleByListIndex(i1).getSceneChanged() &&
    global_stage.getCurrentScene().getParticleByListIndex(i2).getSceneChanged()) && !destroying
